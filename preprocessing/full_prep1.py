...
def savenpy(filelist,prep_folder,data_path,use_existing=True): #id      
    ...
    try:
        ...
        print ('save to: ', (prep_folder+ '/' + name+'_clean'))
...
def full_prep_riqiang(data_root, prep_folder, min_index, max_index, n_worker = None,use_existing=True):
    warnings.filterwarnings("ignore")
    if not os.path.exists(prep_folder):
        os.mkdir(prep_folder)
...
#    shuffle(filelist)
    ...
    for i in range(min_index, max_index):
        try:
            savenpy(filelist=filelist[i], prep_folder=prep_folder,data_path=data_path,use_existing=use_existing)
        except:continue
    #N = len(filelist)
    #_=pool.map(partial_savenpy,range(N))
    #pool.close()
