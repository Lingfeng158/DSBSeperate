...
def step1_python(case_path):
    case = load_scan(os.path.join(case_path, "data"))
    case_pixels, spacing = get_pixels_hu(case)
    case2 = load_scan(os.path.join(case_path, "mask"))
    case_pixels2, spacing2 = get_pixels_hu(case2)
    if((np.sum((spacing==spacing2)*1))!=3):
	print("SPACING ERROR\n")
    ...
    bw = binarize_per_slice(case_pixels2, spacing2)
